...
Pixso MCP Server — a bridge that lets LLMs (e.g., Codex) interact with Pixso design files via the Model Context Protocol. The project has two parts that communicate over WebSocket (port 3667):
...
```
MCP Client (Codex) <--HTTP:3668/mcp--> server/index.js
                                     |
