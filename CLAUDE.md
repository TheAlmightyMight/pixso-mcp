...
- `server/tools.js`: Defines six MCP tools with Zod-validated input schemas. Exports `toolDefinitions` and `handleToolCall(name)`. Includes `buildExportToolResult` (SVG as text, PNG as image), `buildDiagnosticResult`, and `buildPluginDebugResult`.
...
| `get_plugin_debug_log` | `getDebugInfo`     | Returns plugin state, debug event log, or raw node properties for analysis               |
