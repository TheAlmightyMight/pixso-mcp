...
1.  **MCP Server (index.js)**: Локальный Node.js сервер, который реализует протокол MCP и запускает WebSocket-сервер (порт 3001).
...
*   `list_layers`: Список слоев верхнего уровня на текущей странице.
*   `get_node_details`: Подробная информация о конкретном узле (текст, цвета, размеры).
...
node index.js
```
Сервер запустится на `stdio` для MCP и откроет WebSocket-порт `3001`.
