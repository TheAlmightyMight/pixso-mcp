...
1.  **MCP Server (index.js)**: Локальный Node.js сервер, который реализует протокол MCP через SSE (порт 3667) и запускает WebSocket-сервер (порт 3667).
...
Сервер запустится на HTTP-порту `3667` (SSE) для MCP и откроет WebSocket-порт `3667`.
